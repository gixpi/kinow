...
impl AuthenticationService for AuthenticationHandler{
    async fn signup(&self,request:Request<SignupRequest>)->Result<Response<TokenInfo>,Status>{
        let a = services::authentication::signup(&self.postgres_db.as_ref(), &self.redis_db.as_ref(), request.into_inner()).await.map_err(|e| return e.to_status())?;
        Ok(Response::new(a))
    }
