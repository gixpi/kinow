...
pub async fn verify_token(db_pool:&sqlx::Pool<Postgres>,data:VerificationRequest)->Result<VerificationResponse,Error>{
    let token = common::get_token_by_access_token(db_pool,&data.access_token).await?;
    ...
    return Ok(VerificationResponse { device_id: token.device_id, device_type:token.device_type})
}

pub async fn renew_token(db_pool:&sqlx::Pool<Postgres>,data:RenewTokenRequest,expiry:i32)->Result<TokenInfo,Error>{
   let row = sqlx::query("SELECT device_id,device_type FROM tokens WHERE access_token = $1 AND refresh_token = $2")
    ...
    let row = row.unwrap();
    let device_id = row.get::<i32,_>("device_id");
    let device_type = row.get::<String,_>("device_type");
    let token = token::Token::new(device_id, data.ip, expiry, device_type);
...
pub async fn get_tokens(db_pool:&sqlx::Pool<Postgres>,data:Pagination)->Result<Tokens,Error>{
    let rows = sqlx::query("SELECT access_token,refresh_token,device_id,device_type,token_status,ip,created_at,access_token_expire_at,refresh_token_expire_at FROM tokens OFFSET $1 LIMIT $2")
    .bind(data.offset)
    .bind(data.limit)
    .fetch_all(db_pool)
    .await
